...
app.post("/deleteses", (req, res) => {
    const sessionID = req.body.sessionID;
    const sqlSelect = "Delete from session where session_id = ?";
    db.query(sqlSelect, [sessionID], (err, result) => {
        if (err == null) {
            res.send("deleted");
        }
...
app.post("/insertses", (req, res) => {
    ...
    const sqlSelect = "Insert into meetings (sessionID,meetDate,session_type) values(?,?,?)";
    db.query(sqlSelect, [sessionID,meetDate,session_type], (err, result) => {
        console.log(err);
        res.send("inserted");
    });
});
...
app.post("/updateses", (req, res) => {
    ...
    const meetdate = req.body.meetdate;
    const sqlSelect = "UPDATE sessions SET meetdate = ? WHERE session_id = ?";
    db.query(sqlSelect, [meetdate, session_id], (err, result) => {
        res.send("update");
    });
});
app.listen(3001, () => {
