...
app.post("/insertses", (req, res) => {
  ...
  //console.log(pledges);
  const sqlInsert =
    "Insert into sessions (course_id,session_type,date,time,session_name, creator_id) values(?,?,?,?,?,?)";
  db.query(
    sqlInsert,
    [course_id, session_type, date, time, session_name, creator_id],
    (err, result) => {
      if (err != null) {
        res.send(err);
        console.log(err);
        ...
        const sqlGetId =
          "SELECT session_id FROM sessions ORDER BY session_id DESC LIMIT 1"; //get last  created session
        db.query(sqlGetId, (err, result) => {
          let session_id = result[0].session_id;
          //get all the students that the session will be applicable to
          const sqlGetStudents =
            "select user_id from sessions left join session_link on sessions.session_id=session_link.session_id left join course_link on sessions.course_id=course_link.course_id left join student_link on course_link.pro_id=student_link.program_id where sessions.session_id=?;";
          db.query(sqlGetStudents, [session_id], (err, result) => {
            ...
            for (let i = 0; i < result.length; i++) {
              ...
              const sqlInsert =
                "Insert into actions (student_id, tables, table_id, seen, date, action_desc) values (?, ?, ?, ?, ?, ?)"; //send notification to every student
              db.query(
                sqlInsert,
                [student_id, table, tableID, seen, date, actionDesc],
                (err, result) => {
